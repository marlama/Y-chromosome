...
#SBATCH --job-name=chrY_MSSNG_db7_3
#SBATCH -N 1 -c 32 --mem 32g --time 10:00:00

module load bcftools plink/2.3dev plink/1.9.beta3a tabix
...
#plink2 --bfile /hpf/largeprojects/tcagstor/scratch/marlam/MSSNG/MSSNG_db7/mssng_db7_3_chrY --recode vcf --out ${pop}_chrY_ISOGG
...
#bgzip ${pop}_chrY_ISOGG.vcf

#tabix -p vcf ${pop}_chrY_ISOGG.vcf.gz

#SplitBiallelic Sites
bcftools norm -m -any ${pop}_chrY_ISOGG.vcf.gz --threads 32 -Oz -o ${pop}_chrY_ISOGG_norm.vcf.gz

bcftools index ${pop}_chrY_ISOGG_norm.vcf.gz --threads 32

#Fit Reference Sites
bcftools norm -Oz --check-ref s -f /hpf/largeprojects/tcagstor/users/marlam/ReferenceFiles/hg38.fa --threads 32 -o ${pop}_chrY_ISOGG_norm_Ref.vcf.gz ${pop}_chrY_ISOGG_norm.vcf.gz
